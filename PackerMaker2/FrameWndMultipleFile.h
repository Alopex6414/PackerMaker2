/*
*     COPYRIGHT NOTICE
*     Copyright(c) 2017~2018, Team Shanghai Dream Equinox
*     All rights reserved.
*
* @file		FrameWndMultipleFile.h
* @brief	This Program is PackerMaker2 Project.
* @author	alopex
* @version	v1.00a
* @date		2018-09-01
*/
#pragma once

#ifndef __FRAMEWNDMULTIPLEFILE_H_
#define __FRAMEWNDMULTIPLEFILE_H_

// Include Common Header File
#include "Common.h"

// Class Definition
class CFrameWndMultipleFileUI : public CContainerUI
{
public:
	CFrameWndMultipleFileUI();

};

class CFrameWndMultipleFileListUI : public IListCallbackUI
{
public:
	LPCTSTR GetItemText(CControlUI* pControl, int iIndex, int iSubItem);

};

extern CFrameWndMultipleFileListUI g_cCFrameWndMultipleFileListUI;

#endif // !__FRAMEWNDMULTIPLEFILE_H_
